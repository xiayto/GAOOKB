...
# ---------------------node2vec graph-------------
	...
	Graph=node2vec.Graph(G,True,1,1)
	Graph.preprocess_transition_probs()
	n2v_neighbors = Graph.simulate_walks(1,16, grelations)
...
def train(args,m,xp,opt):
	...
	for positive, negative in generator_train_with_corruption(args):
		loss = m.train(positive,negative,glinks,grelations,gedges,xp,n2v_neighbors)
		loss.backward()
		opt.update()
	...
	for mode in ['dev','test']:
		...
		for batch in  chunked(current_data, args.test_batch_size):
			with chainer.using_config('train',False), chainer.no_backprop_mode():
				current_score = m.get_scores(batch,glinks,grelations,gedges,xp,mode,n2v_neighbors)
...
def argument():
	...
	p.add_argument('--nn_model',		'-nn',  default='n2vA')
	...
	p.add_argument('--walk_length',	"-wL",	default=16,		type=int)
	p.add_argument('--N2V_size',	"-NS",	default=1,		type=int)
