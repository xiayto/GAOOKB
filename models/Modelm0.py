...
class Tunnel(chainer.Chain):
	def __init__(self, dim, dropout_rate, activate, layer, isR, isBN, relation_size, pooling_method,heads_size):
		...
		self.forwardAA=Attention(heads_size,dim)
...
class Model(chainer.Chain):
	def __init__(self, args):
		...
		linksB = [('b{}'.format(i), Tunnel(args.dim,args.dropout_block,args.activate,args.layerR,args.is_residual,args.is_batchnorm, args.rel_size, args.pooling_method,args.heads_size)) for i in range(args.order)]
		for link in linksB:
			self.add_link(*link)
