...
class Model(chainer.Chain):
	...
	def get_context2(sefl,entities,glinks2,relations,edges,order,xp):
		if self.depth==order:
			return self.embedE(xp.array(entities,'i'))
		...
		for i,e in enumerate(entities):
			if e in glinks2:
				if len(glinks2[e])<=self.sample_size:	nn = glinks2[e]
				else:		nn = random.sample(glinks[e],self.sample_size)
			for k in nn:
				if k[0] not in neighbor_dict:
					neighbor_dict[k[0]] = len(neighbor_dict)
				assign[neighbor_dict[k[0]]].append(i)
		neighbor = []
		for k,v in sorted(neighbor_dict.items(),key=lambda x:x[1]):
			neighbor.append(k)
		x = self.get_context2(neighbor,glinks2,relations,edges,order+1,xp)
		x = getattr(self,self.forwardC[order][0])(x,neighbor,neighbor_dict,assign,entities,relations)
		return x
	...
	def train(self,positive,negative,links,relations,edges,xp,glinks2):
		self.cleargrads()
	...
	def get_scores(self,candidates,links,relations,edges,xp,mode,glinks2):
		entities = set()
