...
class Attention(chainer.Chain):
	...
	def __call__(self,x,neighbor,neighbor_dict,assign,entities,relations):
	...
	def attentionpooling(self,v,x,assign,eij,neighbor_dict):
		for i in range(len(assign[v])):
			if(i==0):
				x[v]=x[neighbor_dict[assign[v][i]]]*eij[0][i]
			else:
				x[v]=x[v]+x[neighbor_dict[assign[v][i]]]*eij[0][i]
		return x[v]
...
class Model(chainer.Chain):
	...
	def get_context(self,entities,links,relations,edges,order,xp,n2v_neighbors):
		...
		for i in range(len(entities)):
			x=sum(self.embedE(xp.array(n2v_neighbors[entities[i]],'i')))/16
