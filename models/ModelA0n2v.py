...
class Model(chainer.Chain):
	...
	def get_context(self,entities,links,relations,edges,order,xp):
		...
		assign1 = defaultdict(list)
		assign2 = defaultdict(list)
		assign3 = defaultdict(list)
		...
		for i,e in enumerate(entities):
			...
			for k in nn:
				...
				if (entities[i],k) in relations:	r = relation(entities[i],k)*2
				else：								r = relation(k,entities[i])*2+1
				assign1[neighbor_dict[k]].append((i,r))
				for as
